...
protected:
  ...
  pstream_common(pstream_common &&rhs) noexcept
      : command_(std::move(rhs.command_)), buf_(std::move(rhs.buf_)) {
    /* derived class is responsible for ios_type::move(rhs) happening */
  }

  pstream_common &operator=(pstream_common &&rhs) noexcept {
    command_ = std::move(rhs.command_);
    buf_ = std::move(rhs.buf_);
    return *this;
  }

  void swap(pstream_common &rhs) noexcept {
...
public:
  ...
  basic_ipstream(basic_ipstream &&rhs)
      : istream_type(std::move(rhs)), pbase_type(std::move(rhs)) {
...
public:
  ...
  basic_opstream(basic_opstream &&rhs)
      : ostream_type(std::move(rhs)), pbase_type(std::move(rhs)) {
...
public:
   ...
   */
  template <typename T, typename = stringable<T>>
  explicit basic_pstream(std::initializer_list<T> l,
                         pmode mode = pstdout | pstdin)
      : basic_pstream(argv_type(l.begin(), l.end()), mode) {}

  basic_pstream(basic_pstream &&rhs)
      : iostream_type(std::move(rhs)), pbase_type(std::move(rhs)) {
...
public:
   ...
   */
  template <typename T, typename = stringable<T>>
  explicit basic_rpstream(std::initializer_list<T> l,
                          pmode mode = pstdout | pstdin)
...
inline basic_pstreambuf<C, T>::basic_pstreambuf()
    : ppid_(-1) // initialise to -1 to indicate no process run yet.
      ,
      wpipe_(-1), wbuffer_(), rbuffer_(), rbufstate_(), rsrc_(rsrc_out),
      status_(-1), error_(0) {
 ...
 */
template <typename C, typename T>
inline basic_pstreambuf<C, T>::basic_pstreambuf(const std::string &cmd,
                                                pmode mode)
    : ppid_(-1) // initialise to -1 to indicate no process run yet.
      ,
      wpipe_(-1), wbuffer_(), rbuffer_(), rbufstate_(), rsrc_(rsrc_out),
 ...
 */
template <typename C, typename T>
inline basic_pstreambuf<C, T>::basic_pstreambuf(basic_pstreambuf &&rhs) noexcept
    : basic_streambuf(static_cast<const basic_streambuf &>(rhs)),
      ppid_(rhs.ppid_),
      wpipe_(rhs.wpipe_), rpipe_{rhs.rpipe_[0], rhs.rpipe_[1]},
      wbuffer_(rhs.wbuffer_), rbuffer_{rhs.rbuffer_[0], rhs.rbuffer_[1]},
      rbufstate_{rhs.rbufstate_[0], rhs.rbufstate_[1], rhs.rbufstate_[2]},
      rsrc_(rhs.rsrc_), status_(rhs.status_), error_(rhs.error_) {
  rhs.ppid_ = -1;
  rhs.wpipe_ = -1;
...
template <typename C, typename T>
inline void basic_pstreambuf<C, T>::swap(basic_pstreambuf &rhs) noexcept {
 ...
 * @relates basic_pstreambuf
 */
template <int N> inline void close_fd_array(pstreams::fd_type (&fds)[N]) {
...
bool basic_pstreambuf<C, T>::fill_buffer(bool non_blocking) {
  ...
  if (non_blocking) {
    ...
    if (flags != -1) {
      ...
      error_ = 0;
      rc = read(rbuf + pbsz, (std::streamsize)bufsz - (std::streamsize)pbsz);
 ...
 */
template <typename C, typename T>
inline std::streamsize basic_pstreambuf<C, T>::write(const char_type *s,
                                                     std::streamsize n) {
...
template <typename C, typename T> inline int pstream_common<C, T>::close() {
  if (!buf_.close())
    this->setstate(std::ios_base::failbit);
  return buf_.status();
}
 ...
 */
template <typename C, typename T>
inline std::size_t pstream_common<C, T>::fopen(FILE *&fin, FILE *&fout,
                                               FILE *&ferr) {
